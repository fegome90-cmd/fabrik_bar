...
**Business Domain:** Claude Code Plugin for Statusline and Hook Notifications

**User Role:** Claude Code AI assistant providing real-time feedback via hook-based notifications

**Key Concepts:** Hook system integration, JSON stdin/stdout communication, layered architecture (infrastructure/domain), fail-safe error handling, git-based status tracking
...
- `hooks/` → `lib/hook_utils.py` via stdin reading and JSON output utilities
- `hooks/` → `lib/git.py` for git repository state queries
- `hooks/` → `lib/config.py` for loading `fabrik_bar.local.md` settings
- `lib/validator.py` → `lib/config.py` for threshold and git event validation
- `lib/notifier.py` → `lib/logger.py` via 4-tier fallback logging
- `hooks/` → `lib/models.py` via TypedDict type definitions for hook data
- `tests/` → All modules via black-box testing at boundaries

**Architecture Notes:**
- Layered architecture: hooks (infrastructure) → lib (domain) with clear separation
- All hooks use shared `hook_utils.py` for stdin/stdout JSON communication
- Logger provides fallback chain: stderr → file → __stderr__ → stdout
- Configuration is fail-safe with validator ensuring valid thresholds and git events
...
- `/read ~/.claude/plugins/fabrik_bar/README.md` - View plugin documentation
- `/hookify list` - List all configured hookify rules
- `/hookify configure` - Enable or disable hookify rules interactively

> **Note:** fabrik_bar operates as a background hook notification system. Hooks automatically trigger on Claude Code events (pretooluse, posttooluse, userpromptsubmit). Configure via `fabrik_bar.local.md` for threshold and git event customization.
